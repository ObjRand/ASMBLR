""" 
ASMBLR: 
A simple language with fucking no purpose that i made up the concept for in an hour or less idrk /srs /j

STD libs: convert, system, fun, math

Data Types: 
  Array, Float, String, Int, Bool (Standard Data Types) <ALMOST DONE>

External Keywords:
  Conversion Functions (USE "convert"):
    TS
    TI
    TF

  Array Keywords:
    APND (Append, example: APND "h",array), <DONE>
    IDX (Get from array with index, example: IDX 2, array), <DONE>
    RMV (Remove from array with index and shifts items in array, example: RMV 3, array) <DONE>

Internal Keywords (included in the lang):
  Logical operators:
    (==, >=, <=, !=, >, <) <DONE>

    (= is also to Define a Variable), <DONE>

    IF, <DONE>
    "," <DONE>
    +, -, /, *, (PLUS, MINUS, DIVIDE, MULTIPLY) <DONE>
    !, (COMMENT SYMBOL) <DONE>
    :, (DEFINE LABEL) <DONE>

  Keywords For Defining:
    GOTO <DONE>,

    Labels <DONE>:
      A label (example: ":Label") is a defined part in your code that can be accessed with GOTO, 
      allowing you execute code from the label at anytime. 
      It must be closed with an END, and you can't define a Function in a Label, but you can call one in a Label.

      GOTO is a function allowing you to go to the start of any Label in the code.

    END <DONE> is a keyword to define the end of a Label.

  PRINT is a keyword for... PRINTING... DUH! <DONE>

example (Hello World) <DONE>:
  !Hello World in ASMBLR
  PRINT "Hello World!"
...
More examples in full docs! 

Importing lib example: IMPORT "math",<LOCAL>
